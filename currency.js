...
const initiallyDisplayedCurrencies = ["USD", "EUR", "GBP", "SEK", "PHP"];
let baseCurrency;
...
function populateAddCurrencyList(){
    for(let i=0; i<currencies.length; i++){
        addCurrencyList.insertAdjacentHTML("beforeend", `<li data-currency=${currencies[i].abbreviation}><img src=${currencies[i].flagURL} class="flag"><span>${currencies[i].abbreviation} - ${currencies[i].name}</span></li>`);
    }
}
...
.then(data=>{
    console.log(data);  
    document.querySelector(".date").textContent = "Senast Uppdaterad: " +data.date;
    data.rates["EUR"] = 1;
